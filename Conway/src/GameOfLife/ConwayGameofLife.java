...
class DrawGrid extends JComponent{
	...
	public void paint(Graphics g) {
		...
		for(int i = 0; i <= 35; i++) {
		g.drawLine(0, (i*20), 980, (i*20));
		}
		for(int j = 0; j <= 49; j++) {
		g.drawLine((j*20), 0, (j*20), 700);
			}
...
public class ConwayGameofLife {
	...
	static ArrayList<Pixel> toUpdate = new ArrayList<Pixel>();
	...
	public static void main(String[] args) throws InterruptedException {
		...
		lifeBoard.setDefaultCloseOperation(lifeBoard.EXIT_ON_CLOSE);
		//lifeBoard.setLocationRelativeTo(null);
		...
		for(int i = 0; i < gridSize+14; i++) {
	    	...
	    	for(int j = 0; j < gridSize; j++) {
	    		...
	    		pixelGrid[i][j].setLocation((i*20),j*20);
	    		/**if(i%2==0||j%2==0) {
